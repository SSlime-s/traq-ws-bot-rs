...
/// 指定したタグの情報を取得します。
pub async fn get_tag(configuration: &configuration::Configuration, tag_id: &str) -> Result<crate::openapi::models::Tag, Error<GetTagError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/tags/{tagId}", local_var_configuration.base_path, tagId=crate::openapi::apis::urlencode(tag_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
